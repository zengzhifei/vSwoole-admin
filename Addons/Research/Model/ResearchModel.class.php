...
/*
    ...
    */
    ...
    function castReseach($data){
        ...
        if($result){
            action_log('add_research', 'member', $data['userid'], $data['userid']);
        }
        if(C('IS_CACHE')){
            $this->Memcache->updateCache();
        }
    ...
    public function getTopic($field='*',$reseachid){
        if(C('IS_CACHE')){
            $cachename = 'getTopic_'.$reseachid;
            $topiclist = $this->Memcache->getCache($cachename);
            if(!$topiclist){
                $TopicModel = M('topic');
                $where['researchid'] = array('eq',$reseachid);
                $topiclist=$TopicModel->field($field)->where($where)->order("id DESC")->select();
                $this->Memcache->setCache($cachename, $topiclist);
            }
        }else{
            $TopicModel = M('topic');
            $where['researchid'] = array('eq',$reseachid);
            $TopicModel->field($field)->where($where)->order("id DESC")->select();
        }
    ...
    public function getTopicInfo($id){
        if(C('IS_CACHE')){
            $cachename = 'getTopicInfo_'.$id;
            $Topicinfo = $this->Memcache->getCache($cachename);
            if(!$Topicinfo){
                $topicModel = M('topic');
                $topicoptionModel = M('topic_option');
                ...
                $this->Memcache->setCache($cachename, $Topicinfo);
            }
        }else{
            $topicModel = M('topic');
            $topicoptionModel = M('topic_option');
    ...
    public function saveInteractCount($where,$data){
        ...
        if($Countinfo){
            $res=$model->where($where)->setInc('count');
        }else{
            $data['count'] = intval(1);
    ...
    public function updateTopicNumber($where,$num){
        $topicModel = M('topic');
        $result = $topicModel->where($where)->setInc('partake_number',$num);
        return $result;
    }
