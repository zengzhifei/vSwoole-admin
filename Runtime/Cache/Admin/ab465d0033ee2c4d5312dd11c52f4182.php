                    ...
                    <div class="col-md-12">
                        ...
                        <div class="portlet box blue">
                            ...
                            <div class="portlet-body">
                                <button style="margin-bottom:5px;" class="btn blue" type="button" id="server-reload-all">全部重载
                                </button>
                                <button style="margin-bottom:5px;" class="btn blue" type="button" id="server-close-all">全部关闭
                                </button>
...
<script>
    $(function () {
        ...
        function getServers() {
            $.ajax({
                url: "<?php echo U('WebSocket/getServers');?>",
                type: 'GET',
                dataType: 'json',
                complete: function () {
                    setTimeout(function () {
                    ...
                    if (res.status === 1) {
                        $.each(res.data, function (k, v) {
                            html += `<tr>
                                         ...
                                         <td class="server_online">${v['server_online']}</td>
                                         <td>
                                            <a><i class="glyphicon glyphicon-refresh server-reload" title="重载"></i></a>
                                            <a><i class="glyphicon glyphicon-off server-close" title="关闭"></i></a>
                                        </td>
                                      </tr>`;
                        });
                    } else {
        ...
        function reloadServer(server_ip) {
            $.ajax({
                url: "<?php echo U('WebSocket/reloadServer');?>",
                type: 'GET',
                ...
                success: function (res) {
                    ...
                    if (res.status === 1) {
                        ...
                        Interact.success_report('服务重载失败');
                    }
                },
                error: function (e) {
                    console.log(e.statusText);
        ...
        function shutdownServer(server_ip) {
            $.ajax({
                url: "<?php echo U('WebSocket/shutdownServer');?>",
                type: 'GET',
                data: {server_ip: server_ip},
                dataType: 'json',
                success: function (res) {
                    console.log(res);
                    if (res.status === 1) {
                        Interact.success_report('服务关闭成功');
                    } else {
                        Interact.success_report('服务关闭失败');
                    }
                },
                error: function (e) {
                    console.log(e.statusText);
