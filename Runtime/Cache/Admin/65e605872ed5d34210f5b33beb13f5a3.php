...
<script>
    ...
    $('.sub-menu').each(function() {
								...
								<div class="form-body">
									<div class="form-group">
										...
										<div class="col-md-2" style="margin-right:70px;">
											<input type="text" placeholder="请输入用户昵称或者ID" name="nickname" class="form-control input-medium search-input" value="<?php echo I('nickname');?>">
										</div>
                                       
										<div class="col-md-2">
											<button class="btn blue" type="button" url="<?php echo U('index');?>" id="search">查询</button>
										</div>
									</div>
								</div>
							</form>
				...
				<div class="col-md-12">
					...
					<div class="portlet box blue">
						...
						<div class="portlet-body">
            			    <button style="margin-bottom:5px;" class="btn blue" type="button" onclick="addurl()">新 增</button>
				            <button style="margin-bottom:5px;" class="btn blue" onclick="batchChangeStatus(this)" url="<?php echo U('changeStatus?method=resumeUser');?>">启 用</button>
				            <button style="margin-bottom:5px;" class="btn blue" onclick="batchChangeStatus(this)" url="<?php echo U('changeStatus?method=forbidUser');?>">禁 用</button>
				            <button style="margin-bottom:5px;" class="btn blue" onclick="batchDelete(this)" url="<?php echo U('changeStatus?method=deleteUser');?>">删 除</button>
							...
							<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
						            ...
						            <td><input type="checkbox" name="checkList" value="<?php echo ($vo["uid"]); ?>"></td>
									...
									<td><?php if(($vo["status"]) == "1"): ?><a href="javascript:void(0);" url="<?php echo U('User/changeStatus?method=forbidUser&id='.$vo['uid']);?>" onclick="changeStatus(this)">禁用</a>
										<?php else: ?>
										<a href="javascript:void(0);" url="<?php echo U('User/changeStatus?method=resumeUser&id='.$vo['uid']);?>" onclick="changeStatus(this)">启用</a><?php endif; ?>
										<a href="<?php echo U('AuthManager/group?uid='.$vo['uid']);?>" class="authorize">授权</a>
						                <a href="javascript:void(0);" onclick="deleteUser(this)" rel="<?php echo ($vo["uid"]); ?>">删除</a>
						                </td>
		...
		//批量修改状态
		function batchChangeStatus(obj){
			var idstr = getCheckBoxValue();
			var url = $(obj).attr('url');
			if(idstr==''){
				$.confirm({title:"请选择要操作的数据",modal: true,level:"warning",buttons: {  
					...
					$.confirm({  
					   ...
					   buttons: {  
						   "确定": {
							   ...
							   "action":function() {
								   $.ajax({
										url:"<?php echo U('changeStatus?method=deleteUser');?>",
										type:'post',
										data:{id:id},
										dataType:'json',
										success:function(msg){
