                    ...
                    <div class="col-md-12">
                        ...
                        <div class="portlet box blue">
                            ...
                            <div class="portlet-body">
                                ...
                                <button style="margin-bottom:5px;" class="btn blue" type="button" id="group-both-open" stage_id="<?php echo ($data['pageParam']['stage_id']); ?>">开启所有分组</button>
                                <button style="margin-bottom:5px;" class="btn blue" type="button" id="group-both-close" stage_id="<?php echo ($data['pageParam']['stage_id']); ?>">关闭所有分组</button>
                                <table class="table table-striped table-hover table-bordered" id="sample_editable_1">
                                    ...
                                    <?php if(!empty($data['pageInfo'])): if(is_array($data['pageInfo'])): foreach($data['pageInfo'] as $key=>$vo): ?><tr>
                                                ...
                                                <td><?php echo ($vo["group_type_name"]); ?></td>
                                                <!--<td><?php echo ($vo["group_time"]); ?></td>-->
                                                <td><?php echo (date("Y-m-d H:i:s",$vo["group_updated"])); ?></td>
                                                <td><?php echo ($vo["group_remark"]); ?></td>                                                      
                                                <!--<?php if($vo["group_init"] == '1'): ?><td style="color: red;">初始化</td>
                                                <?php else: ?>
                                                    <td>未初始化</td><?php endif; ?>  
                                                <td>
	                                                <?php if($vo["is_false"] == '1'): ?><a href="javascript:void(0);" id="<?php echo ($vo["id"]); ?>" status=0  onclick="changeStatus(this)">关闭</a>
	                                                <?php else: ?>
	                                                    <a href="javascript:void(0);" id="<?php echo ($vo["id"]); ?>" status=1  onclick="changeStatus(this)">开启</a><?php endif; ?>
                                                </td>     -->
                                                <td>
                                                    <?php if($vo["group_status"] == 0): ?><a><i group_id="<?php echo ($vo["id"]); ?>" group_init="<?php echo ($vo["group_init"]); ?>" group_status="1" class="glyphicon glyphicon-play group-switch" title="点击开启本轮次"></i></a>
                                                    <?php else: ?>
                                                        <a><i group_id="<?php echo ($vo["id"]); ?>" group_init="<?php echo ($vo["group_init"]); ?>" group_status="0" class="glyphicon glyphicon-stop group-switch" title="点击关闭本轮次"></i></a><?php endif; ?>
                                                    <!--<a><i group_id="<?php echo ($vo["id"]); ?>" stage_id="<?php echo ($vo["stage_id"]); ?>" class="glyphicon glyphicon-ok group-init" title="初始化"></i></a>-->
                                                    <a><i group_id="<?php echo ($vo["id"]); ?>" class="glyphicon glyphicon-edit group-edit" title="编辑"></i></a>
                                                    <a><i group_id="<?php echo ($vo["id"]); ?>" stage_id="<?php echo ($vo["stage_id"]); ?>" class="glyphicon glyphicon-wrench group-maintenance" title="维护数据"></i></a>
                                                    <a><i group_id="<?php echo ($vo["id"]); ?>" group_status="<?php echo ($vo["group_status"]); ?>" class="glyphicon glyphicon-trash group-delete" title="删除"></i></a>
                                                    <!--<a class="glyphicon glyphicon-export group-export" title="导出" href="<?php echo U('Group/exportList',array('group_id'=>$vo['id']));?>"></a>-->
                                                </td>
...
<script>
    $(function () {
        ...
        $("#group-add").on('click', function () {
            var url = "<?php echo U('Group/addEdit');?>" + "&stage_id=" + "<?php echo ($data['pageParam']['stage_id']); ?>";
        ...
        $(".group-maintenance").on('click', function () {
            ...
            var url = "<?php echo U('Qa/index');?>" + '&stage_id=' + stage_id + '&group_id=' + group_id;
            window.location.href = url;
        })
        //编辑数据
        $(".group-edit").on('click', function () {
            var group_id = $(this).attr('group_id');
            var url = "<?php echo U('Group/addEdit');?>";
        ...
        function changeStatus(obj){
            var url = "<?php echo U('Group/changeStatus');?>";
            var id = $(obj).attr("id");
            ...
            $.confirm({
                ...
                buttons: {
                    "确定": {
                        ...
                        action: function () {
                            $.ajax({
                                url: "<?php echo U('Group/initGroup');?>",
                                type: 'POST',
                                dataType: 'JSON',
                                data: data,
                                timeout: 20000,
                                        ...
                                        $.confirm({
                                            ...
                                            buttons: {
                                                "确定": {
                                                    ...
                                                    action: function () {
                                                        if (msg.status == -2) {
                                                            window.location.href = "<?php echo U('Player/index');?>";
                                                        }
                                                    }
                                                }
                                            }
            ...
            $.confirm({
                ...
                buttons: {
                    "确定": {
                        ...
                        action: function () {
                            $.ajax({
                                url: "<?php echo U('Group/switchGroup');?>",
                                type: 'POST',
                                dataType: 'JSON',
                                data: data,
                                timeout: 20000,
             ...
             $.confirm({
                ...
                buttons: {
                    "确定": {
                        ...
                        action: function () {
                            $.ajax({
                                url: "<?php echo U('Group/switchAllGroup');?>",
                                type: 'POST',
                                dataType: 'JSON',
            ...
            $.confirm({
                ...
                buttons: {
                    "确定": {
                        ...
                        action: function () {
                            $.ajax({
                                url: "<?php echo U('Group/deleteGroup');?>",
                                type: 'GET',
