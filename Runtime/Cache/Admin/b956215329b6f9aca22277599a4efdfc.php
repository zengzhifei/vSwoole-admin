...
<script src="/demo/vSwoole-admin/Public/Home/interact/js/jquery.form.js" type="text/javascript"></script>
<div class="clearfix">
...
<script>
    ...
    $('.sub-menu').each(function() {
								...
								<div class="form-body">
									<div class="form-group">
										...
										<div class="col-md-2" style="margin-right:70px;">
											<input type="text" placeholder="请输入菜单名称" name="title" class="form-control input-medium search-input" value="<?php echo I('title');?>">
										</div>
                                       
										<div class="col-md-2">
											<button class="btn blue" type="button" url="/demo/vSwoole-admin/admin.php?s=/Menu/index/pid/157.html" id="search">查询</button>
										</div>
									</div>
								</div>
							</form>
            ...
            <div class="row">
				<div class="col-md-12">
					...
					<div class="portlet box blue">
						<div class="portlet-title">
							<div class="caption">
								<i class="fa fa-edit"></i><?php if(isset($data)): ?>[ <?php echo ($data["title"]); ?> ] 子<?php endif; ?>菜单管理
							</div>
							<!-- <div class="tools">
						...
						<div class="portlet-body">
            			    <button style="margin-bottom:5px;" class="btn blue" onclick="addurl(this)" url="<?php echo U('add',array('pid'=>I('get.pid',0)));?>">新 增</button>
            			    <button style="margin-bottom:5px;" class="btn blue" onclick="batchDelete(this)" url="<?php echo U('del');?>">删 除</button>
				            <!-- <button style="margin-bottom:5px;" class="btn blue" onclick="changeStatus(this)" url="<?php echo U('import',array('pid'=>I('get.pid',0)));?>">导 入</button> -->
				            <button style="margin-bottom:5px;" class="btn blue" onclick="sortList(this)" url="<?php echo U('setSort');?>">排序</button>
				            
							<form action="<?php echo U('setSort');?>" method="post" id="sortForm">
							...
							<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$menu): $mod = ($i % 2 );++$i;?><tr>
			                        <td><input type="checkbox" name="checkList" value="<?php echo ($menu["id"]); ?>"></td>
			                        <td><input type="text" size="4" style="text-align:center;" placeholder="排序" name="sort[<?php echo ($menu["id"]); ?>]" value="<?php echo ($menu["sort"]); ?>"></td>
			                        ...
			                        <td>
			                            <a href="javascript:void(0);" onclick="isShow(this)" rel="<?php echo U('toogleDev',array('id'=>$menu['id'],'value'=>abs($menu['is_dev']-1)));?>">
			                            <?php echo ($menu["is_dev_text"]); ?>
			                            </a>
			                        </td>
			                        <td>
			                            <a href="javascript:void(0);" onclick="isShow(this)" rel="<?php echo U('toogleHide',array('id'=>$menu['id'],'value'=>abs($menu['hide']-1)));?>">
			                            <?php echo ($menu["hide_text"]); ?>
			                            </a>
			                        </td>
			                        <td>
			                            <a title="编辑" href="<?php echo U('edit?id='.$menu['id']);?>">编辑</a>
			                            <a href="javascript:void(0);" onclick="deleteMenu(this)" rel="<?php echo ($menu['id']); ?>">删除</a>
			                        </td>
		...
		<!-- 
		<script type="text/javascript" src="/demo/vSwoole-admin/Public/static/think.js"></script>-->

         
		<script src="/demo/vSwoole-admin/Public/static/thinkbox/jquery.thinkbox.js"></script>
