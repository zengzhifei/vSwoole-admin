...
class HttpController extends AdminController
{
    /**
     * 管理页
     */
    ...
    {
        $curl = new Curl();
        $server_list = $curl->get(C('Http.Get_Servers'));
    ...
    {
        $curl = new Curl();
        $res = $curl->get(C('Http.Clear_Servers'));
    ...
    public function reloadServer()
    {
        ...
        $curl = new Curl();
        $res = $curl->get(C('Http.Reload_Server'), ['server_ip' => $server_ip]);
        if (($res = json_decode($res, true)) && $res['status'] == 1) {
            $this->ajaxReturn(['status' => 1, 'msg' => 'success']);
        } else {
            $this->ajaxReturn(['status' => 0, 'msg' => 'fail']);
    ...
    public function shutdownServer()
    {
        $server_ip = I('server_ip', null);
        $curl = new Curl();
        $res = $curl->get(C('Http.ShutDown_Server'), ['server_ip' => $server_ip]);
        if (($res = json_decode($res, true)) && $res['status'] == 1) {
            $this->ajaxReturn(['status' => 1, 'msg' => 'success']);
        } else {
            $this->ajaxReturn(['status' => 0, 'msg' => 'fail']);
